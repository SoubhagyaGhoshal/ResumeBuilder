...
const _anchorId = (tokens, idx) => Number(tokens[idx].meta.id + 1).toString();
const _anchorLabel = (tokens, idx) => tokens[idx].meta.label;
const _isOpen = (state, start) => {
  return (
    state.src.charCodeAt(start) === 0x5b /* [ */ &&
    state.src.charCodeAt(start + 1) === 0x7e /* ~ */
  );
};
const _isClose = (state, pos) => {
  return state.src.charCodeAt(pos) === 0x5d /* ] */;
};
const _extractLabel = (state, start, end) => {
  return state.src.slice(start, end);
};
const render = (type) => (tokens, idx) => {
...
const _processDefToken = (state, startLine, endLine, pos, label) => {
  state.env.crossRef ??= {};
...
const processDef = (state, startLine, endLine, silent) => {
 ...
 * ```
 */
export const MarkdownItCrossRef = (md) => {
