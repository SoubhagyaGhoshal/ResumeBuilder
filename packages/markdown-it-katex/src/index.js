...
const isValidDelim = (state, pos) => {
  ...
  // 0x20: " ", 0x09: \t
  const isWhitespace = (char) => char === 0x20 || char === 0x09;
  // 0x30: "0", 0x39: "9"
  const isDigit = (char) => char >= 0x30 && char <= 0x39;
 ...
 */
export const MarkdownItKatex = (md, options = { throwOnError: false }) => {
  const renderKatex = (
    tex,
    options,
    displayMode // Whether to render as block math
  ) => {
